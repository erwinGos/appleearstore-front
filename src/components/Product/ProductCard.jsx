  ...
  return (
    <>
      ...
      <ul onMouseOver={() => setShowHideBtn(true)} onMouseLeave={() => setShowHideBtn(false)} className='transition duration-500 ease-in-out transform hover:scale-105 hover:shadow-lg border-2 rounded w-auto h-auto flex flex-col justify-end'>
           <Link to={'/productdetails/' + product.product.id}><li><img src={ProductImg} alt="" className="" /></li></Link>
            { user.isAuth ?
          ...
          <li className='pl-2 pt-2'><h4 className="text-lg md:text-xl lg:text-2xl">{product.product.productName}</h4></li>
          <li className='pl-2'><p className="text-base md:text-lg">{product.product.brand.name}</p></li>
